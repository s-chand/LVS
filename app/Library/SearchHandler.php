...
class SearchHandler
{
    public static function searchByParcelNumber($parcelNumber,$user_id,$search_text,$search_time)
    {
        ...
        if (is_numeric($parcelNumber)) {
             ...
             */
            $sql='SELECT DISTINCT ba.name_firstpart,ba.name_lastpart,p.name,p.last_name,p.email,p.address,ba.location,ba.type_code from party AS p
  inner join party_for_rrr AS prr on prr.party_id=p.id
  inner join rrr AS r on r.id=prr.rrr_id
  inner join ba_unit AS ba on ba.id=r.ba_unit_id where ba.name_firstpart = ?';
